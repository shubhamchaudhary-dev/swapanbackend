...
exports.generateAndUploadCertificate = void 0;
const sharp_1 = __importDefault(require("sharp"));
const crypto_1 = __importDefault(require("crypto"));
const pdf_lib_1 = require("pdf-lib");
const path_1 = __importDefault(require("path"));
const cloudinary_1 = require("./cloudinary");
const uploadBufferToCloudinary = (buffer, folder, filename, isPdf) => {
 ...
 */
function extractAuthorNamesList(authors) {
    const result = [];
...
const generateAndUploadCertificate = async (title, authors, journalName, publicationDate, paperSlug) => {
    const templatePath = path_1.default.join(__dirname, '../assets/certificate.png');
    // Template is exactly 1536 x 1024 px
    const W = 1536;
    const H = 1024;
    const authorNamesList = extractAuthorNamesList(authors);
    const pdfDoc = await pdf_lib_1.PDFDocument.create();
    let firstPngBuffer = null;
    ...
    const titleSvg = titleLines.map((line, i) => `<text x="${centerX}" y="${Math.round(titleBaselineY + i * titleLineH)}" ` +
        `font-family="Georgia, 'Times New Roman', serif" font-size="${titleFontSize}" ` +
        `font-weight="bold" fill="#0B1F5C" text-anchor="middle">${line}</text>`).join('\n');
    // ──────────────────────────────────────────────────────────────────────────
  ...
  `;
    ...
    for (const authorName of authorNamesList) {
        const certId = `SP-${new Date().getFullYear()}-${crypto_1.default.randomBytes(4).toString('hex').toUpperCase()}`;
        ...
        const authorsSvg = authorsLines.map((line, i) => `<text x="${centerX}" y="${Math.round(authorsBaselineY + i * authorsLineH)}" ` +
            `font-family="Georgia, 'Times New Roman', serif" font-size="${authorsFontSize}" ` +
            `font-weight="normal" fill="#1a1a4e" text-anchor="middle">${line}</text>`).join('\n');
      ...
      ${certIdSvg}
        ...
        const generateAndUploadForAuthor = async () => {
            const pngBuffer = await (0, sharp_1.default)(templatePath)
                .composite([{ input: Buffer.from(svgOverlay), top: 0, left: 0 }])
                .png()
                .toBuffer();
            const authorPdfDoc = await pdf_lib_1.PDFDocument.create();
